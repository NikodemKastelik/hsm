...
static bool s11_handler(hsm_event_t evt);
static bool s2_handler(hsm_event_t evt);

static const hsm_state_t s1  = {.handler = s1_handler,  .parent = NULL};
static const hsm_state_t s11 = {.handler = s11_handler, .parent = &s1};
...
bool s11_handler(hsm_event_t evt)
{
    switch (evt)
    {
        case HSM_EVENT_ENTRY:
